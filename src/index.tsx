...
export const useSnackBar = (
  props: SnackBarProps = { fontSize: 14, align: 'bottom' },
  force = false
) => {
  ...
  useEffect(() => {
    ...
    if (!force && style.boxSizing && style.boxSizing !== 'border-box') {
      console.warn(
        'react-snack-bar: box-sizing is not border-box, it may produce some problems'
      );
    }
    const div = document.createElement('div');
    document.body.appendChild(div);
    ReactDOM.render(
      <SnackBar ref={ref} align={align} fontSize={fontSize}></SnackBar>,
      div
    );
    return () => {};
  }, []);
  return {
    show: (text: string, type: Type = 'ERROR') =>
      ref.current && ref.current.show(text, type),
  };
};
